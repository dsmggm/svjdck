...
function savesettings() {
    ...
    // 创建一个 JSON 对象
    var data = { crontab_time: crontab_time, apptoken: apptoken, gg: gg };
    // 发送 POST 请求
    ...
    .then(response => {
        response.json().then(data => {
            if (response.status === 200) {
                ...
                element3.value = gg;
                const element4 = document.getElementById('ver');
                element4.textContent = `当前版本：${ver}`;
                const element5 = document.getElementById('auth_date');
                element5.textContent = `授权到期日期：${auth_date}`;
